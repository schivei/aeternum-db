...
internal static class CatalogPersistedDataTypeJsonCodec
{
    public static string Serialize(PersistedDataTypeSnapshot payload) =>
        JsonSerializer.Serialize(
            payload,
            CatalogPersistedDataTypeJsonContext.Default.PersistedDataTypeSnapshot
        );
    ...
    public static bool TryParseDataType(string text, out DataType type)
    {
        ...
        try
        {
            ...
            {
                "array" => new DataType.Vector(
                    CatalogDataTypePersistenceConverter.ParseDataType(
                        payload.ElementTypeText
                            ?? CatalogDataTypePersistenceConstants.UnknownTypeName,
                        null
                    )
                ),
                "reference" => new DataType.Reference(
                    payload.Table
                        ?? payload.DataId
                        ?? CatalogDataTypePersistenceConstants.UnknownTypeName
                ),
                "reference_array" => new DataType.ReferenceArray(
                    payload.Table
                        ?? payload.DataId
                        ?? CatalogDataTypePersistenceConstants.UnknownTypeName
                ),
                ...
                "user_defined" => new DataType.EnumRef(
                    payload.Name
                        ?? payload.DataId
                        ?? CatalogDataTypePersistenceConstants.UnknownTypeName
                ),
