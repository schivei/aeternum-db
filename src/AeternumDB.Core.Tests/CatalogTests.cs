...
public sealed class CatalogTests
{
    ...
    public void TableLifecycle_AlterAndIndexRegistry_Works()
    {
        ...
        var indexCreated = catalog.CreateIndex(
    ...
    public void RemoveType_WhenReferencedByTable_Throws()
    {
        ...
        catalog.AddType(
            new UserTypeSchema(
                "status",
                new UserTypeKind.Enum(
                    false,
                    [new EnumVariant("OPEN"), new EnumVariant("CLOSED")],
                    [1, 2]
                )
            )
        );
        ...
        try
        {
            ...
            catalog.AddType(
                new UserTypeSchema(
                    ...
                    new UserTypeKind.Enum(
                        false,
                        [new EnumVariant("LOW"), new EnumVariant("HIGH")],
                        [1, 2]
                    )
                )
            );
            catalog.CreateTable(
                new TableSchema(
                    ...
                    [
                        ...
                        new ColumnSchema(
                            "assignee_refs",
                            new DataType.ReferenceArray("users"),
                            nullable: true
                        ),
                        new ColumnSchema(
                            "tags",
                            new DataType.Vector(new DataType.Varchar(50)),
                            nullable: true
                        ),
                    ]
                )
            ...
            Assert.IsType<DataType.Varchar>(
                reloaded.GetTable("tasks")!.GetColumn("name")!.DataType
            );
            Assert.IsType<DataType.Decimal>(
                reloaded.GetTable("tasks")!.GetColumn("ratio")!.DataType
            );
            Assert.IsType<DataType.Reference>(
                reloaded.GetTable("tasks")!.GetColumn("assignee_ref")!.DataType
            );
            Assert.IsType<DataType.ReferenceArray>(
                reloaded.GetTable("tasks")!.GetColumn("assignee_refs")!.DataType
            );
        ...
        Parallel.For(
            ...
            i =>
            {
                var created = catalog.CreateTable(
                    new TableSchema(
                        $"t_{i}",
                        [new ColumnSchema("id", DataType.Integer.Instance, nullable: false)]
                    ),
                    ifNotExists: true
                );
                Assert.True(created);
    ...
    public void CatalogPersistence_DataTypeRoundtrip_RestoresJsonBackedTypes()
    {
        ...
        try
        {
            ...
            catalog.CreateTable(
                new TableSchema(
                    ...
                    [
                        ...
                        new ColumnSchema(
                            "profile_ref",
                            new DataType.VirtualReference("profiles", "id"),
                            nullable: true
                        ),
                        new ColumnSchema(
                            "profile_refs",
                            new DataType.VirtualReferenceArray("profiles", "id"),
                            nullable: true
                        ),
                        new ColumnSchema(
                            "flags",
                            new DataType.Vector(DataType.Integer.Instance),
                            nullable: true
                        ),
            ...
            var ownerRef = Assert.IsType<DataType.Reference>(
                table!.GetColumn("owner_ref")!.DataType
            );
            Assert.Equal("users", ownerRef.Table);

            var ownerRefs = Assert.IsType<DataType.ReferenceArray>(
                table.GetColumn("owner_refs")!.DataType
            );
            Assert.Equal("users", ownerRefs.Table);

            var profileRef = Assert.IsType<DataType.VirtualReference>(
                table.GetColumn("profile_ref")!.DataType
            );
            Assert.Equal("profiles", profileRef.Table);
            Assert.Equal("id", profileRef.Column);

            var profileRefs = Assert.IsType<DataType.VirtualReferenceArray>(
                table.GetColumn("profile_refs")!.DataType
            );
    ...
    public void CatalogPersistence_LoadsLegacyDataTypeTextFormats()
    {
        ...
        try
        {
            ...
            var ownerRef = Assert.IsType<DataType.VirtualReference>(
                table.GetColumn("owner_ref")!.DataType
            );
            Assert.Equal("users", ownerRef.Table);
            Assert.Equal("id", ownerRef.Column);

            var owners = Assert.IsType<DataType.ReferenceArray>(
                table.GetColumn("owners")!.DataType
            );
            Assert.Equal("users", owners.Table);

            var profileRef = Assert.IsType<DataType.VirtualReferenceArray>(
                table.GetColumn("profile_ref")!.DataType
            );
            Assert.Equal("profiles", profileRef.Table);
            Assert.Equal("id", profileRef.Column);
