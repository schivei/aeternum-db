...
namespace AeternumDB.PoC.Safe.Query;

/// <summary>
/// Translates <see cref="PhysicalPlan"/> nodes into executable <see cref="IExecutionPlan"/>
/// operator trees using fully safe operators.
/// Shares <see cref="ExprEvaluator"/> and <see cref="Accumulators"/> from the shared project.
/// Mirrors Rust: executor/physical_plan.rs build_executor() — safe/AOT variant.
/// </summary>
[Injectable<IQueryPlanner>(ServiceLifetime.Singleton)]
public sealed class SafeQueryPlanner : IQueryPlanner
{
    ...
    public static IExecutionPlan BuildExecutor(PhysicalPlan plan, ITableProvider provider) =>
        plan switch
        {
            ...
            PhysicalPlan.Limit lim =>
                new SafeLimitExec(BuildExecutor(lim.Input, provider), lim.LimitCount, lim.Offset),
            ...
            PhysicalPlan.Distinct distinct =>
                new SafeDistinctExec(BuildExecutor(distinct.Input, provider)),

            PhysicalPlan.HashAggregate agg =>
                new SafeHashAggregateExec(
            ...
            PhysicalPlan.Unnest unnest =>
                new SafeUnnestExec(BuildExecutor(unnest.Input, provider),
                    unnest.ArrayColumn, unnest.OutputColumn),

            _ => new SafeValuesExec([], []),
        };
